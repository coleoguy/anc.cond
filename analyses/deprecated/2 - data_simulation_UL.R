...
set.seed(42)
########## 0) BD trees (Simulate and Rescale) ###########
...
for (i in seq_along(sizes)) {
  ...
  # 2. Rescale each tree to unit height (1.0) using an inner loop
  for (j in 1:reps) {
    current_tree <- raw_trees[[j]]
    
    # Calculate max height and divide edge lengths
    max_h <- max(phytools::nodeHeights(current_tree))
    current_tree$edge.length <- current_tree$edge.length / max_h
    
    # Put the rescaled tree back
...
# Clean up: Remove everything from memory EXCEPT 'bd_trees'
...
save(sim_data,
     file = "../data/sim_data_UL.RData")
message("Saved simulated data to /data/sim_data_UL.RData")
